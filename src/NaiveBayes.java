...
class NaiveBayes {
	...
	//IntegerRating[][][] trainingRatings;
	int[][][] trainingRatings;
	
	double globalAverage;
	//HashMap <String, HashMap<String, Integer>> trainingRatings;
	
	/*
	public double crossValidateAndReturnRMSLE(ArrayList<Rating> allRatings, int numberOfFolds) {
		Collections.shuffle(allRatings);
		
		int ratingsPerFold = allRatings.size() / numberOfFolds;
		
		ArrayList<HashSet<Rating>> folds = new ArrayList<HashSet<Rating>>();
		int i;
		HashSet<Rating> tmp = new HashSet<Rating>();
		for (i = 0; i < numberOfFolds-1; i++) {
			tmp = new HashSet<Rating>();
		...
		i = 0;
		for (HashSet<Rating> leaveOutForPrediction : folds) {		
			watch.start();
			
			trainingRatings = new HashMap <String, HashMap<String, Integer>>();
			for (Rating rating : allRatings) {
				if (!leaveOutForPrediction.contains(rating)) {
					if (!trainingRatings.containsKey(rating.userId)) {
						trainingRatings.put(rating.userId, new HashMap<String, Integer>());
					}
					trainingRatings.get(rating.userId).put(rating.productId, rating.ratingValue);
				}
			}
			
			double rmsle = 0;
			for (Rating target : leaveOutForPrediction) {
				rmsle += Math.log(target.ratingValue) - Math.log(predict(target.userId, target.productId));
			}
			rmsle /= leaveOutForPrediction.size();
	...
	*/
	...
	private double getConditionalProbability(int productId1, int ratingValue1, int productId2, int ratingValue2) {
		...
		// TODO: Make structure for this if too slow and if so no need to store userIds anymore, 
